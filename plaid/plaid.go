...
type TransactionRequest struct {
	...
	Options     *TransactionRequestOptions `json:"options,omitempty"`
}
...
func (c *Client) transactions(accessToken string, startDate, endDate time.Time, options TransactionRequestOptions) (TransactionResponse, error) {
...
func (c *Client) Transactions(accessToken string, startDate, endDate time.Time) (TransactionResponse, error) {
	...
	resp, err := c.transactions(accessToken, startDate, endDate, options)

	if err != nil {
		return resp, err
	}

	seenNum := len(resp.Transactions)
	totalNum := resp.TotalTransactions

	if seenNum == totalNum {
		return resp, nil
	}

	// Fetch all transactions in range with repeated requests
	allTransactions := make([]Transaction, totalNum)
	copy(allTransactions[0:], resp.Transactions)

	for seenNum < totalNum {
		options = TransactionRequestOptions{Offset: seenNum}
		resp, err = c.transactions(accessToken, startDate, endDate, options)
		...
		copy(allTransactions[seenNum:], resp.Transactions)
		seenNum += len(resp.Transactions)
	}
...
func (c *Client) RetrieveBalance(accessToken string) (BalanceResponse, error) {
